...
@implementation AudioMobileOggVorbisEncoder
...
-(void) encodeWav:(NSURL*)wavURL toOggVorbisDestination:(NSURL*)oggURL {
    ...
    NSString* wavFilePath = [wavURL path];
    NSString* vorbFilePath = [oggURL path];
    
    wavIn = fopen([wavFilePath UTF8String], "r");
    if (!wavIn) {
        NSLog(@"wave file does not exist.");
        return;
    }
    
    vorbOut = fopen([vorbFilePath UTF8String], "w");
    if (!vorbOut) {
        NSLog(@"failed to fopen vorbis out file.");
        return;
    }
    ...
    if(ret) {
        NSLog(@"Failed to setup ogg vorbis encoder");
        if ([self delegate]) {
            [[self delegate] encodingCompletedWithStatus:AMENCODINGFAILURE];
            return;
        }
    };
    ...
    while(!eos){
        ...
        if (firstIter) {
            //we want to fade in the beginning of the stream, so zero the first few bytes until we reach a natural zero byte; this will eliminate any pop at the beginning of the file
            for(int byteI = 0;byteI<bytes;byteI++) {
//                NSLog(@"char: %u",readbuffer[byteI]);
//                if (readbuffer[byteI]!=0) {
//                    readbuffer[byteI] = (signed char)0;
//                }
//                else {
//                    //we've encountered the first natural 0, so end fading
//                    firstIter = false;
//                    break;
//                }
                //This is a kludge:  for whatever reason our encoding process is inserting a sequence of 6 bytes into the recorded audio which causes a severe pop at the beginning of the playback.  This sets those 6 bytes to 0.
                if (readbuffer[byteI]==100) {
                    if (readbuffer[byteI+1] == 97 && readbuffer[byteI+2] == 116 && readbuffer[byteI+3] == 97) {
                        readbuffer[byteI] = 0;
                        readbuffer[byteI+1] = 0;
                        readbuffer[byteI+2] = 0;
                        readbuffer[byteI+3] = 0;
                        readbuffer[byteI+4] = 0;
                        readbuffer[byteI+5] = 0;
                        firstIter = false;
                    }
                    readbuffer[byteI] = (signed char)0;
                }
...
//            firstIter = false;
    ...
    if ([self delegate]) {
        [[self delegate] encodingCompletedWithStatus:AMENCODINGSUCCESS];
    }
